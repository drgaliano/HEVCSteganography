...
% Date: 2020/04/07
% Function: step1_algorithm_random
% 
% Purpose: Adds a stego-message inside a YUV video format (luminance
% matrix) in 4x4 Insertion Blocks (IBs), applying random algorithm.
% 
...
COORDINATES_PATH = ['/path/to/video-modified/coord_random_' FILE_NAME(1:end-4) '.txt'];

% Path and filename where modified YUV video format will be located.
YUV_PATH_WITH_HIDDEN_MSG = ['/path/to/video-modified/toEncode_random_' FILE_NAME];
...
[message, messageLength]=textToBin(MSG_PATH);

% Generate random coordinates.
coordinates=random_matrix(COORDINATES_PATH, messageLength, FRAMES, YUV_WIDTH, YUV_HEIGHT);
...
for i=1 : messageLength*CHAR_SIZE
    ...
    for j=0 : BLOCK_SIZE-1
        for k=0 : BLOCK_SIZE-1
            ...
            Y_BinValue(BIT_TO_CHANGE) = message(letter, bit);         % Modifies the affected bit.

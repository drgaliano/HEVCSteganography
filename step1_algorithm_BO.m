...
% Date: 2020/04/07
% Function: step1_algorithm_BO
% 
% Purpose: Adds a stego-message inside a YUV video format (luminance 
% matrix) in 4x4 Insertion Blocks (IBs), applying BO algorithm.
% 
...
COORDINATES_PATH = ['/path/to/video-modified/coord_BO_per70_' FILE_NAME(1:end-4) '.txt'];

% Path and filename where modified YUV video format will be located.
YUV_PATH_WITH_HIDDEN_MSG = ['/path/to/video-modified/toEncode_BO_per70_' FILE_NAME];

% Pixel depth.
PIXEL_DEPTH = 8;

% Char size in bits
CHAR_SIZE = 8; %Tamano del char en bits.
...
% Maximum number of random tuples that are generated for each bit to add.
MAX_ITERATIONS = 10;

% Acceptable percentage of 1 or 0 equal to the bit under study for an IB to
% be valid. The value must be between 0 and 1.
ACCEPTABLE_PERCENTAGE = 0.7;
...
% Generates BO coordinates.
coordinates=BO_matrix(MAX_ITERATIONS, ACCEPTABLE_PERCENTAGE, COORDINATES_PATH, PIXEL_DEPTH, BIT_TO_CHANGE, CHAR_SIZE, BLOCK_SIZE, FRAMES, binMessage, messageLength, YUV_WIDTH, YUV_HEIGHT, Y);
